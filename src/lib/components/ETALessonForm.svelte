<!-- // src/lib/components/ETALessonForm.svelte -->
<script>
	...
	function copyToClipboard() {
		if (finalLessonPlanOutput) {
			navigator.clipboard
				...
				.then(() => {
					alert('ETA Lesson plan copied to clipboard!');
				})
				.catch((err) => {
					console.error('Failed to copy: ', err);
					alert('Failed to copy ETA lesson plan to clipboard.');
				});
		}
	}
</script>
...
>
	<div class="form-group">
		<div class="input-group">
			<input
				...
				required
			/>
			<label for="eta-topic" class="input-label"> Lesson Topic (ETA) </label>
		</div>
	</div>
		...
		<div class="form-group">
			<div class="input-group">
				<input
					...
					required
				/>
				<label for="eta-classDuration" class="input-label"> Class Duration (ETA) </label>
			</div>
		</div>
		<div class="form-group">
			<div class="input-group">
				...
				<label for="eta-grade" class="input-label"> Grade Level (ETA) </label>
			</div>
		</div>
	</div>
	...
	<div class="form-group">
		<div class="input-group">
			...
			<label for="eta-coTeachingModel" class="input-label"> Co-teaching Model (ETA) </label>
		</div>
	</div>
	...
	<div class="form-group">
		<div class="input-group">
			<textarea
				...
				required
			></textarea>
			<label for="eta-objectives" class="input-label"> Learning Objectives (ETA) </label>
		</div>
	</div>
	...
	<div class="form-group">
		<div class="input-group">
			...
			<label for="eta-classDescription" class="input-label">
				Classroom Context (ETA) <span class="text-gray-500">(Optional)</span>
			</label>
		</div>
